...
import type { CoreVerticlesProps } from "@/types/features/verticles";
import { Icon } from "@/components/shared/icon";

function CoreVerticles({
  heading,
  description,
  verticles,
}: CoreVerticlesProps) {
      ...
      <Container>
        <div className="flex flex-col gap-8 lg:gap-10">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {verticles.map(({ id, uuid, image, title, ...verticle }) => (
              <Link key={uuid} href={`/verticles/${uuid}`} className="block">
                <Card
                  {...verticle}
                  title={title}
                  ...
                  classNames={{
                    mainWrapper:
                      "h-[280px] max-w-none rounded-[12px] border-transparent bg-white shadow-[0_4px_12px_#00000040] transition-shadow hover:shadow-[0_6px_18px_#00000059]",
