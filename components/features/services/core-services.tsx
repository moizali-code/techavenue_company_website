      ...
      <Container>
        <div className="flex flex-col gap-8 lg:gap-10">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {services.map(
              ({ id, uuid, articleUuid, image, title, ...service }) => (
                <Link key={uuid} href={`#${articleUuid}`} className="block">
                  <Card
                    {...service}
                    title={title}
                    ...
                    classNames={{
                      mainWrapper:
                        "h-[220px] max-w-none rounded-[12px] border-transparent bg-white shadow-[0_2px_8px_#0000001A] transition-shadow hover:shadow-[0_4px_12px_#0000002E]",
                      content: "justify-center gap-2 p-2",
                      eyebrow: "mb-1",
                      title:
