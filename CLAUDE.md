...
- **`uuid` identifies, `id` does not**: Every data entity carries both an `id` and a `uuid`. Unique identification, lookups, route params, keys, and login are always based on `uuid` — `id` is decorative and has no real usage in the project. Name props and variables accordingly: `activeUuid`, not `activeId`; `industryUuid`, not `industryId`. Never route or match on `id`.
